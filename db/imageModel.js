var db = require('./imageModel.js');
var mongoose = require('mongoose');

var imageSchema = mongoose.Schema({
  url: String,
  title: String,
  rating: String
});

var Image = mongoose.model('Image', imageSchema);

var firstImg = { id: 0, url: 'http://www.freedigitalphotos.net/images/img/homepage/87357.jpg', title: 'Duck', rating: 3};

// var imageData = [
//   {
//     id: 0,
//     url: 'http://www.freedigitalphotos.net/images/img/homepage/87357.jpg',
//     title: 'Duck',
//     rating: 3
//   },
//   {
//     id: 1,
//     url: 'http://www.gettyimages.pt/gi-resources/images/Homepage/Hero/PT/PT_hero_42_153645159.jpg',
//     title: 'Jellyfish',
//     rating: 4
//   },
//   {
//     id: 2,
//     url: 'http://www.spyderonlines.com/images/wallpapers/images/images-22.jpg',
//     title: 'Bear',
//     rating: 1
//   },
//   {
//     id: 3,
//     url: 'http://eskipaper.com/images/images-4.jpg',
//     title: 'Dog',
//     rating: 5
//   },
//   {
//     id: 4,
//     url: 'https://upload.wikimedia.org/wikipedia/commons/b/b9/P.t.altaica_Tomak_Male.jpg',
//     title: 'Tiger',
//     rating: 5
//   }
// ];

// Image.create(imageData, function(response) {
//   console.log('First image saved:', response);
// });

module.exports = Image;

